...
class MobileNet():
    def __init__(self, input_size : list, num_classes : int):
...
class MobileNet_RS():
    def __init__(self, input_size : list, num_classes : int):
        ...
        self.inputs = tf.keras.Input(shape = input_size)
        self.x = self.base_model(self.inputs, training = True)
        self.x = tf.keras.layers.Dense(num_classes)(self.x)
        self.outputs = tf.keras.layers.Lambda(restrictedSoftmax)(self.x)
...
def get_convnext_model(
    ...
    x = layers.Dense(num_classes, name="classification_head")(x)
    outputs = layers.Lambda(restrictedSoftmax)(x)
